...
public class DummyComparer : IEqualityComparer<GenerationData>
{
    private static string GetData(GenerationData data)
    {
#pragma warning disable RS1035
        return $"Namespace: {data.ContainingNamespace.ToDisplayString()}{Environment.NewLine}" +
               $"Type: {data.TargetType.ToDisplayString()}{Environment.NewLine}" +
               $"Class: {data.Node.Identifier.ValueText}{Environment.NewLine}";
    }
#pragma warning restore RS1035
    public bool Equals(GenerationData x, GenerationData y)
    {
        Generator.Log("Starting Comparison");
        Generator.Log($"X: {GetData(x)}");
        Generator.Log($"Y: {GetData(y)}");
        return true;
    }

    public int GetHashCode(GenerationData obj)
    {
        Generator.Log("CompareH");
        return 1;
    }
}
...
{
#pragma warning disable RS1035
    public static void Log(string message) => File.AppendAllText(
        "/Users/matysz/repos/TestBuilderGenerator/log.txt",
...
#pragma warning restore RS1035
    ...
    {
#pragma warning disable RS1035
        File.WriteAllText("/Users/matysz/repos/TestBuilderGenerator/log.txt", $"START{Environment.NewLine}");
#pragma warning restore RS1035
        ...
        var provider = context.SyntaxProvider
            ...
            .SelectMany((x, _) => x)
            .Select((a, b) => a)
            .WithComparer(new DummyComparer());
    ...
    private void Execute(SourceProductionContext context, GenerationData data)
    {
        ...
        indentWriter.WriteLine($"// {DateTime.Now}");
        indentWriter.WriteLine("#pragma warning disable CA5394");

        if (!data.ContainingNamespace.IsGlobalNamespace)
        {
            // TODO: not file scoped namespaces
            indentWriter.WriteLine($"namespace {data.ContainingNamespace.ToDisplayString()};");
...
#pragma warning restore S125
            ...
            var hasAlreadyDefinedDefaultProperty =
                builderProperties.Any(x => x.Identifier.ValueText == defaultPropertyName);
            if (!hasAlreadyDefinedDefaultProperty)
            {
                ...
                switch (propertyType.Name)
                {
                    ...
                    case not "" when propertyType.ContainingNamespace.ToDisplayString()
...
#pragma warning restore S1144
        (Compilation Compilation, ImmutableArray<GenerationData> GenerationData) touple)
    {
        var sb = new StringBuilder();
        var writer = new StringWriter(sb);
        using var indentWriter = new IndentedTextWriter(writer);

        foreach (var data in touple.GenerationData)
        {
            if (IsNotPartial(data.Node))
            {
                EmitErrorDiagnostic(context, data.Node);
...
#pragma warning restore S125
        ...
        CancellationToken cancellationToken)
    {
        ...
        return new GenerationData(classDeclaration, context.TargetSymbol.ContainingNamespace, targetType);
    }
